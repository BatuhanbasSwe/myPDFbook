...
describe('extractLines', () => {
  it('aynı taban çizgisindeki parçaları tek satırda, yukarıdan aşağıya sıralar', () => {
    const { lines } = extractLines(
      0,
      page([
        item('ikinci satır', 50, 480, 60),
        item('Birinci', 50, 500, 35),
        item('satır', 90, 500, 25),
      ]),
    );
  ...
  it('bitişik parçaları boşluksuz birleştirir (kelime ortası bölünme)', () => {
    const { lines } = extractLines(
      0,
      page([
        item('Sabahın ilk ışık', 50, 500, 70),
        item('ları kasa', 120, 500, 40),
        item('banın', 160, 500, 25),
      ]),
    );
  ...
  it('üst simgeyi (dipnot işareti) aynı satıra alır', () => {
    const { lines } = extractLines(
      0,
      page([
        item('tutuyordu.', 50, 500, 50),
        item('¹', 100.5, 503.5, 3, 6),
        item('Sonra', 60, 485, 30),
      ]),
    );
  ...
  it('döndürülmüş metni ve boş parçaları atlar', () => {
    ...
    const { lines } = extractLines(
      0,
      page([rotated, item('', 50, 500, 0), item(' ', 45, 500, 3), item('metin', 50, 500, 25)]),
    );
  ...
  it('satır ölçülerini hesaplar', () => {
    const { lines } = extractLines(
      0,
      page([item('Başlık', 100, 520, 80, 16), item('metin', 40, 500, 30, 10)]),
    );
